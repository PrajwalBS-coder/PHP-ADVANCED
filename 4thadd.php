...
$q =mysqli_query($con, "select * from add WHERE passno = $passno ");
        $row=mysqli_fetch_array($q);
    
if($row==0){
$sql = mysqli_query($con,"INSERT INTO add VALUES('$passno','$fname','$mname','$lname','$dob','$nation','$addr','$gender','$fstore')");
if($sql)
                    {
                        echo '<script>alert("Data Added Successfully");window.location="4thv.php"</script>';
                    }
                    else
                    {
    ...
    echo '<script>alert("Passport Number Already Present");window.location="p4add.php"</script>';
}
}
?>
</body>
